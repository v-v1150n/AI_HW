...
def sentence3() -> Expr:
    ...
    s1 = conjoin(PacmanAlive_1 % ((PacmanAlive_0 & ~PacmanKilled_0) | (~PacmanAlive_0 & PacmanBorn_0)))
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return True if not findModel(premise & (~conclusion)) else False
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    clauses = [disjoin(~item[0], ~item[1]) for item in itertools.combinations(literals, 2)]
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    temp_list = [PropSymbolExpr(wall_str, x, y) >> (~PropSymbolExpr(pacman_str, x, y, time=t)) for (x, y) in all_coords]
    pacphysics_sentences.append(conjoin(temp_list))

    temp_list_2 = [PropSymbolExpr(pacman_str, x, y, time=t) for (x, y) in non_outer_wall_coords]
    pacphysics_sentences.append(exactlyOne(temp_list_2))

    temp_list_3 = [PropSymbolExpr(action, time=t) for action in DIRECTIONS]
    pacphysics_sentences.append(exactlyOne(temp_list_3))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        temp_list = [PropSymbolExpr(pacman_str, x, y, time=t) for (x, y) in non_wall_coords]
        KB.append(exactlyOne(temp_list))

        temp_list_2 = [PropSymbolExpr(action, time=t) for action in actions]
        KB.append(exactlyOne(temp_list_2))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        KB.append(exactlyOne(temp_list_2))
        if t != 0:
            KB += [pacmanSuccessorAxiomSingle(x, y, time=t, walls_grid=walls) for (x, y) in non_wall_coords]
        goal = [~PropSymbolExpr(food_str, x, y, time=t) for (x, y) in food]
        model = findModel(conjoin(conjoin(KB), conjoin(goal)))
        if model != False:
            return extractActionSequence(model, actions)
        for (x, y) in food:
            KB.append((PropSymbolExpr(food_str, x, y, time=t) & PropSymbolExpr(pacman_str, x, y, time=t)) >> ~PropSymbolExpr(food_str, x, y, time=t + 1))
            KB.append(~PropSymbolExpr(pacman_str, x, y, time=t) >> (PropSymbolExpr(food_str, x, y, time=t + 1) % PropSymbolExpr(food_str, x, y, time=t)))
