...
def constructBayesNet(gameState: hunters.GameState):
    ...
    extended_range = max_manhattan_distance + MAX_NOISE
    ...
    variableDomainsDict = {
        ...
        OBS0: list(range(extended_range + 1)),
        OBS1: list(range(extended_range + 1))
    }
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder=None):
        nonlocal callTrackingList  
        if callTrackingList is None:
            callTrackingList = []
        ...
        for var in eliminationOrder:
            if var not in queryVariables and var not in evidenceDict:
                factors_with_var = [f for f in factors if var in f.variablesSet()]
                if factors_with_var:
                    _, joined_factor = joinFactorsByVariable(factors, var)

                    if len(joined_factor.unconditionedVariables()) > 1:
                        new_factor = eliminate(joined_factor, var)
                        factors = [f for f in factors if f not in factors_with_var] + [new_factor]
                    else:
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        total = self.total()
        if total == 0:
            return  # Do nothing if the total is zero
    ...
    def sample(self):
        ...
        total = self.total()
        rand_point = random.uniform(0, total)
        cumulative = 0
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            # If the observation is None and the ghost is in jail, probability is 1
            if noisyDistance is None:
                return 1.0
            # If there is an observation but the ghost is in jail, it should be impossible
            else:
                return 0.0

        # If the observation is None and the ghost is not in jail, it should be impossible
        if noisyDistance is None:
            return 0.0

        # Calculate the true Manhattan distance from Pacman to the ghost
        trueDistance = manhattanDistance(pacmanPosition, ghostPosition)

        # Use the provided function to calculate the probability of the noisy observation given the true distance
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            if observation is None:
                ...
                if pos == jailPosition:
                    newBeliefs[pos] = 0  # Ghost is not in jail if there's a valid observation
                else:
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        newBeliefs = Counter()
        for oldPos in self.allPositions:  # Iterate over all possible positions
            newPosDist = self.getPositionDistribution(gameState, oldPos)  # Get distribution over new positions
            for newPos, prob in newPosDist.items():  # Iterate over new positions and their probabilities
                newBeliefs[newPos] += self.beliefs[oldPos] * prob  # Update belief at new position
        self.beliefs = newBeliefs  # Update beliefs to the new predictions
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        remainder = self.numParticles % num_legal_positions  

        for position in self.legalPositions:  
            num_particles_at_position = particles_per_position  
            if remainder > 0:  
                num_particles_at_position += 1
                remainder -= 1
            self.particles.extend([position] * num_particles_at_position)
    ...
    def getBeliefDistribution(self):
        ...
        "*** END YOUR CODE HERE ***"
        from collections import Counter 
        belief_distribution = Counter()  

        for particle in self.particles:
            belief_distribution[particle] += 1

        total_particles = len(self.particles)
        for position in belief_distribution:
            belief_distribution[position] /= total_particles  
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            observationProb = self.getObservationProb(observation, pacmanPosition, particle, jailPosition)
            weightDistribution[particle] += observationProb
