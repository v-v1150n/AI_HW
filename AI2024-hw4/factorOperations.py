...
def joinFactors(factors: List[Factor]):
    ...
    # Create a new factor with the variables obtained
    new_factor = Factor(list(unconditioned_vars), list(conditioned_vars), factor.variableDomainsDict())

    # Calculate probability entries for the new factor
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        "*** YOUR CODE HERE ***"
        "*** END YOUR CODE HERE ***"
        # Get the unconditioned and conditioned variables after elimination
        ...
        for assignment_dict in new_factor.getAllPossibleAssignmentDicts():
            probability = 0.0
            # Sum the probabilities of rows that match assignments on other variables
            for assignment_dict_factor in factor.getAllPossibleAssignmentDicts():
                match = True
                for variable, value in assignment_dict.items():
                    if variable != eliminationVariable and assignment_dict_factor[variable] != value:
                        match = False
                        break
                if match:
                    probability += factor.getProbability(assignment_dict_factor)
            new_factor.setProbability(assignment_dict, probability)
